...
{{-- Quick Stats --}}
<h3 class="text-xl font-bold mb-4 text-gray-800">Ringkasan Anak</h3>
<div class="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8">
    @foreach($quickStats as $stat)
    <div class="bg-blue-50 rounded-xl p-4 shadow-lg border-b-4 border-{{ $stat['border_color'] }}-500 transform hover:shadow-xl transition duration-200">
        <div class="flex items-center justify-between">
            <p class="text-xs font-semibold uppercase tracking-wider text-gray-700">{{ $stat['title'] }}</p>
            <span class="p-1 rounded-full {{ $stat['color'] }}">
                {!! getSvgIcon($stat['icon'], 'w-5 h-5') !!}
            </span>
        </div>
        <p class="text-3xl font-extrabold text-gray-900 mt-2">{{ $stat['count'] }}</p>
    </div>
    @endforeach
</div>

{{-- Menu Fitur --}}
<h3 class="text-xl font-bold mb-4 text-gray-800">Fitur Utama</h3>
